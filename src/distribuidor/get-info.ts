import atomos from "./info"

const nomeatomo = (num: number) => atomos.nomes[num - 1]
const simboloatomo = (num: number) => atomos.simbolos[num - 1]

function achagrupo(num: number) {
...
function achafamilia(num: number, grupo: number) {
    return (
...
function achaperiodo(num: number) {
...
const getInfo = (num: number) => {
    const grupo = achagrupo(num)
